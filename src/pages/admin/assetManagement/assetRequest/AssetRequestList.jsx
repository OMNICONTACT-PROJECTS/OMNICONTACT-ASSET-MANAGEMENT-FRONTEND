...
export const AssetRequestListLoader = async () => {
  try {
    const response = await assetRequestsServices.getAllByOrganisationId(
...
const AssetRequestList = () => {
  ...
  const assetRequestTableColumns = [
    ...
    },
    {
      ...
      render: (allocatedBy) =>
        allocatedBy
          ? `${allocatedBy.first_name} ${allocatedBy.last_name}`
          : "Not Allocated",
    },
    {
  ...
  const approveAssetRequest = async (record) => {
    ...
    try {
      const response = await assetRequestsServices.approveRequest(
        record.id,
        data
      );
      if (response?.status === 200) {
